...
const Items: Item[] = [
  ...
  {
    title: "Men’s Fashion", href: "/mens-fashion", subItems: [
      {title: "Dresses", href: "/women-fashion/dresses"},
      {title: "Shoes", href: "/women-fashion/shoes"},
    ],
  },
...
const Sidebar = () => {
  ...
  const list = (
    <Box sx={{width: 250}} role="presentation" onKeyDown={toggleDrawer(false)}>
      <List>
        {Items.map((item) => (
          <SidebarItem key={item.title} item={item} openDropdowns={openDropdowns}
  ...
  return (
    <div>
      <Button onClick={toggleDrawer(true)}><VscLayoutSidebarLeft className={'text-3xl text-black'}/></Button>
